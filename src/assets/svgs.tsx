import React, { useEffect, useState } from 'react';

export const Gear = () => {
  const [show, setShow] = useState(false);

  useEffect(() => {
    setShow(true);
    setTimeout(() => {
      // setHide(true);
      setShow(false);
    }, 4000)
  }, [setShow])

  return (
  <svg className='gear-icon' version='1.1' id='Layer_1' xmlns='http://www.w3.org/2000/svg' x='0px' y='0px' viewBox='0 0 328 328'>
    <path className={`gear ${show ? 'show' : ''} `} fill='#fff' d='M325.1,169c1-0.4,1.9-1.6,1.9-2.7c0,0,0-0.4,0-1.7c0-12.2-3.5-33.4-3.5-33.4c-0.2-1.1-1.2-2.1-2.3-2.2l-23.6-2.6
      c-1.1-0.1-2.3-1.1-2.6-2.1l-3.7-10.4c-0.4-1-0.1-2.5,0.6-3.3l16.3-17.9c0.7-0.8,0.9-2.3,0.3-3.2l-19.1-28.7c-0.7-0.9-2-1.3-3.1-0.9
      l-22.1,8.7c-1,0.4-2.5,0.1-3.3-0.7l-9.1-8.4c-0.8-0.7-1.3-2.2-1-3.2l6.1-23.3c0.3-1.1-0.3-2.4-1.2-3l-29.9-16.1
      c-1-0.5-2.4-0.2-3.1,0.7l-15.6,18.1c-0.7,0.8-2.2,1.3-3.2,1l-12.8-3.2c-1.1-0.2-2.2-1.3-2.4-2.4l-5.4-23.3c-0.2-1.1-1.3-2-2.4-2.1
      c0,0-10.5-0.9-16.9-0.9s-16.9,0.9-16.9,0.9c-1.1,0.1-2.2,1.1-2.4,2.1L139.3,28c-0.2,1.1-1.3,2.1-2.4,2.4L124,33.6
      c-1.1,0.3-2.5-0.1-3.2-1l-15.6-18.1c-0.7-0.8-2.1-1.1-3.1-0.7L72.3,30c-0.9,0.6-1.5,1.9-1.2,3l6.1,23.3c0.3,1.1-0.2,2.5-1,3.2
      L67,68c-0.8,0.8-2.3,1.1-3.3,0.7l-22.1-8.7c-1-0.4-2.4,0-3.1,0.9L19.4,89.5c-0.6,1-0.4,2.4,0.3,3.2l16.3,17.9
      c0.7,0.8,1,2.3,0.6,3.3l-3.7,10.4c-0.3,1-1.5,2-2.6,2.1L6.9,129c-1.1,0.1-2.1,1.1-2.3,2.2c0,0-3.5,21.3-3.5,33.4c0,1.2,0,1.7,0,1.7
      c0,1.1,0.9,2.3,1.9,2.7l22.8,8.3c1,0.4,2,1.6,2.1,2.7l1.3,9.2c0.2,1.1-0.4,2.5-1.3,3.1L8.1,205.5c-0.9,0.6-1.4,2-1,3L20.6,242
      c0.5,1,1.8,1.7,2.8,1.5l24.1-3.3c1.1-0.1,2.5,0.5,3.1,1.4l4.5,6.3c0.7,0.9,0.8,2.4,0.3,3.4l-11.4,21c-0.5,1-0.3,2.4,0.5,3.2
      l27.9,23.9c0.9,0.7,2.3,0.7,3.2,0l19.8-14.1c0.9-0.6,2.4-0.7,3.4-0.2l5.9,3c1,0.5,1.8,1.8,1.8,2.9l-0.3,24c0,1.1,0.8,2.3,1.9,2.6
      l36.1,8.6c1.1,0.2,2.4-0.5,2.9-1.4l11-21.6c0.5-1,1.8-1.8,2.9-1.7c0,0,1.4,0,3.1,0s3.1,0,3.1,0c1.1,0,2.4,0.7,2.9,1.7l11,21.6
      c0.5,1,1.8,1.6,2.9,1.4l36.1-8.6c1.1-0.3,1.9-1.5,1.9-2.6l-0.3-24c0-1.1,0.8-2.4,1.8-2.9l5.9-3c1-0.5,2.5-0.4,3.4,0.2l19.8,14.1
      c0.9,0.6,2.3,0.6,3.2,0l27.9-23.9c0.8-0.8,1-2.2,0.5-3.2l-11.4-21c-0.5-1-0.4-2.5,0.3-3.4l4.5-6.3c0.6-0.9,2-1.5,3.1-1.4l24.1,3.3
      c1.1,0.1,2.4-0.5,2.8-1.5l13.5-33.4c0.3-1-0.1-2.4-1-3l-19.8-13.3c-0.9-0.6-1.5-2-1.3-3.1l1.3-9.2c0.1-1.1,1.1-2.3,2.1-2.7
      L325.1,169z M164,264.2c-55.2,0-100-44.8-100-100c0-55.2,44.8-100,100-100s100,44.8,100,100C264,219.4,219.2,264.2,164,264.2z'/>

    <path className={`left-bracket ${show ? 'show' : ''}`} fill='#1a7fe5' d='M120.9,194.4h-4.8c-5.4,0-8.9-0.8-10.8-2.4c-1.8-1.6-2.8-4.8-2.8-9.6v-6.8c0-3.1-0.5-5.2-1.6-6.4c-1.1-1.1-3-1.7-5.9-1.7
      h-2c-0.2,0-0.3-0.1-0.3-0.3v-6.1c0-0.2,0.1-0.3,0.3-0.3h2c2.9,0,4.8-0.6,5.9-1.7c1.1-1.1,1.6-3.3,1.6-6.4v-6.8c0-4.7,0.9-8,2.8-9.6
      c1.9-1.6,5.4-2.4,10.8-2.4h4.8c0.2,0,0.3,0.1,0.3,0.3v6c0,0.2-0.1,0.3-0.3,0.3H117c-2.1,0-3.5,0.4-4.2,1.2c-0.7,0.8-1,2.7-1,5.7
      v6.6c0,3.5-0.5,6.1-1.6,7.5c-0.9,1.2-2.3,2-4.3,2.6c2,0.5,3.4,1.4,4.3,2.6c1.1,1.5,1.6,4,1.6,7.5v6.5c0,3,0.3,4.9,1,5.7
      c0.7,0.8,2.1,1.2,4.2,1.2h3.9c0.2,0,0.3,0.1,0.3,0.3v6.1C121.2,194.3,121.1,194.4,120.9,194.4z'/>
    <path className={`ess ${show ? 'show' : ''}`} fill='#fff' d='M144.6,183.8c-2,0-4-0.2-6-0.7c-2-0.4-4-1.1-5.8-2c-0.1-0.1-0.2-0.2-0.2-0.3v-7.6c0-0.1,0.1-0.2,0.2-0.3
      c0.1-0.1,0.2,0,0.3,0c2.1,1.4,4.1,2.4,6,3.1c1.9,0.7,3.9,1,5.7,1c1.9,0,3.3-0.4,4.3-1.2c1-0.8,1.5-1.8,1.5-3.2c0-1-0.3-1.9-1-2.7
      c-0.7-0.8-1.7-1.4-3-1.9l-3.9-1.4c0,0,0,0,0,0c-4-1.4-6.8-2.9-8.2-4.5c-1.5-1.6-2.2-3.8-2.2-6.5c0-3.5,1.2-6.2,3.6-8.2
      c2.4-2,5.7-3,9.8-3c1.9,0,3.7,0.2,5.6,0.6c1.8,0.4,3.7,1,5.5,1.7c0.1,0,0.2,0.2,0.2,0.3v7.2c0,0.1-0.1,0.2-0.2,0.3
      c-0.1,0.1-0.2,0.1-0.3,0c-1.7-1.1-3.4-1.9-5.1-2.5c-1.7-0.6-3.4-0.9-5.1-0.9c-1.8,0-3.3,0.3-4.2,1c-0.9,0.7-1.4,1.6-1.4,2.8
      c0,0.9,0.3,1.7,1,2.3c0.7,0.6,2.2,1.4,4.5,2.2l3.4,1.2c3.3,1.1,5.7,2.6,7.2,4.5c1.6,1.9,2.4,4.2,2.4,7.1c0,3.8-1.2,6.8-3.7,8.7
      C153.2,182.8,149.5,183.8,144.6,183.8z'/>
    <path className={`emm ${show ? 'show' : ''}`} fill='#fff' d='M195.8,183.1h-7.1c-0.2,0-0.3-0.1-0.3-0.3v-27.6l-4.3,14.7c0,0.1-0.2,0.2-0.3,0.2h-5.5c-0.1,0-0.3-0.1-0.3-0.2l-4.4-14.7
      v27.6c0,0.2-0.1,0.3-0.3,0.3h-7.1c-0.2,0-0.3-0.1-0.3-0.3v-37.2c0-0.2,0.1-0.3,0.3-0.3h9.8c0.1,0,0.3,0.1,0.3,0.2l4.7,15.3
      l4.6-15.3c0-0.1,0.2-0.2,0.3-0.2h9.9c0.2,0,0.3,0.1,0.3,0.3v37.2C196.1,182.9,196,183.1,195.8,183.1z'/>
    <path className={`right-bracket ${show ? 'show' : ''}`} fill='#1a7fe5' d='M211.8,194.4h-4.8c-0.2,0-0.3-0.1-0.3-0.3v-6.1c0-0.2,0.1-0.3,0.3-0.3h3.8c2.1,0,3.5-0.4,4.2-1.2c0.7-0.8,1.1-2.8,1.1-5.7
        v-6.5c0-3.6,0.5-6,1.6-7.5c0.9-1.2,2.3-2.1,4.2-2.6c-2-0.5-3.4-1.4-4.2-2.6c-1.1-1.5-1.6-3.9-1.6-7.5v-6.6c0-2.9-0.4-4.8-1.1-5.6
        c-0.7-0.8-2.1-1.2-4.2-1.2h-3.8c-0.2,0-0.3-0.1-0.3-0.3v-6c0-0.2,0.1-0.3,0.3-0.3h4.8c5.4,0,8.9,0.8,10.7,2.4
        c1.8,1.6,2.8,4.8,2.8,9.6v6.8c0,3.1,0.6,5.2,1.6,6.4c1.1,1.1,3.1,1.7,6,1.7h2c0.2,0,0.3,0.1,0.3,0.3v6.1c0,0.2-0.1,0.3-0.3,0.3h-2
        c-2.9,0-4.9,0.6-6,1.7c-1.1,1.2-1.6,3.3-1.6,6.4v6.8c0,4.7-0.9,8-2.8,9.6C220.7,193.6,217.2,194.4,211.8,194.4z'/>
  </svg>
  )
  };

export const Fingers = () => (
  <span className='icon'>
    <svg
      version='1.1'
      id='Layer_1'
      xmlns='http://www.w3.org/2000/svg'
      x='0px'
      y='0px'
      viewBox='0 0 69 50'
    >
      <path
        fill='#ffffff'
        className='fingers'
        d='M64.2,25.5c-2.5-2.6-14-6.8-14.5-7c-0.3-0.1-0.6-0.2-1-0.2c-1.3,0-2.6,0.9-3.1,2.2c-0.1,0.3-0.2,0.8-0.2,1.2
                c-0.3-0.1-0.6-0.1-1-0.1c-1.5,0-2.9,1-3.4,2.4c-0.3,0.7-0.3,1.4-0.1,2.2l-13.1-4.8c-0.5-0.2-0.9-0.3-1.4-0.3c-1.5,0-2.7,0.9-3.2,2.2
                c-0.7,1.9,0.3,3.8,2.2,4.5l1,0.4c-1.2,0.3-2.1,1.1-2.5,2.2c-0.4,1-0.3,2,0.2,2.9c0.5,0.8,1.2,1.5,2.2,1.8l15.6,5.3
                c0.1,0.5,0.2,1.2,0.7,2.2c1.1,2.7,5,5.4,9.7,5.4c0.9,0,1.9-0.1,2.7-0.3c6.2-1.5,9.7-4.6,11.8-10.4C69.1,31.3,66.7,28.3,64.2,25.5z
                M65.4,37c-1.9,5.3-5,8-10.7,9.4c-0.8,0.2-1.6,0.3-2.4,0.3c-4,0-7.4-2.3-8.3-4.5c-0.2-0.4-0.3-0.8-0.4-1l5.1,1.7l0,0
                c0.4,0.1,0.8-0.1,1-0.5c0.1-0.4-0.1-0.8-0.5-1l-22.4-7.6c-0.6-0.2-1.1-0.6-1.4-1.1s-0.3-1.1-0.1-1.6c0.4-1.1,1.6-1.6,2.9-1.1
                l13.7,4.4c0.4,0.1,0.8-0.1,1-0.5c0.1-0.4-0.1-0.8-0.4-1l-16.6-6.5c-1.1-0.4-1.6-1.5-1.3-2.6c0.3-0.8,0.9-1.2,1.8-1.2
                c0.3,0,0.5,0,0.8,0.1l16,5.8l0,0c0.4,0.1,0.8-0.1,1-0.5c0.1-0.4-0.1-0.8-0.5-1c-0.5-0.2-0.9-0.6-1.2-1.1c-0.2-0.5-0.3-1.1-0.1-1.6
                c0.3-0.8,1.1-1.4,1.9-1.4c0.2,0,0.5,0,0.7,0.1l3.1,1.1c0.4,0.1,0.8-0.1,1-0.5c0.1-0.4-0.1-0.8-0.5-1l-1.9-0.7c0-0.3,0-0.9,0.1-1.3
                c0.3-0.8,1.3-1.4,2-1.1c3.3,1.2,12.1,4.7,13.9,6.6C65.5,29.2,67.3,31.7,65.4,37z'
      />
      <path
        fill='#5AD6FF'
        d='M9.2,41c-0.8,0.1-1.7,0.4-2.5,0.6C3.8,29,4.2,15.7,8,3.2c0.4-0.7-0.1-1.7-0.7-2c-0.7-0.1-1.7,0.4-2,1.1
            C1.4,15.4,1,29.1,3.9,42.3c-1.1,0.3-2.1,0.4-3.2,0.7C0.4,43.1,0,43.2,0,43.8c0,0.6,0.1,0.8,0.6,1.1C2.4,46,4.2,47.4,6,48.3
            c0.6,0.3,1.3,0.1,1.4-0.4c0.8-2,1.8-3.8,2.9-5.6c0.3-0.4,0.3-0.7,0-1C10.1,41,9.7,41,9.2,41z'
      />
    </svg>
  </span>
);

export const Logo = () => (
  // <svg
  //   version='1.1'
  //   id='Layer_1'
  //   xmlns='http://www.w3.org/2000/svg'
  //   x='0px'
  //   y='0px'
  //   viewBox='-23.9 -16.1 64 64'
  // >
  //   <circle
  //     id='XMLID_27_'
  //     className='logo-circle'
  //     cx='8.4'
  //     cy='15.5'
  //     r='28.4'
  //   />
  //   <g id='XMLID_35_'>
  //     <line
  //       id='XMLID_28_'
  //       className='logo-lines'
  //       x1='5.9'
  //       y1='25.5'
  //       x2='11.5'
  //       y2='5.9'
  //     />
  //     <path
  //       id='XMLID_33_'
  //       className='logo-lines'
  //       d='M-1.8,8.6c-2.2,2.2-4.7,4.7-6.9,6.9l6.9,6.2'
  //     />
  //     <path
  //       id='XMLID_34_'
  //       className='logo-lines'
  //       d='M18.6,22.1c2.2-2.2,4.7-4.7,6.9-6.9l-6.9-6.2'
  //     />
  //   </g>
  // </svg>
  <svg version='1.1' id='Layer_1' xmlns='http://www.w3.org/2000/svg' x='0px' y='0px' viewBox='0 0 1920 1080'>
    <path fill='#1a7fe5' d='M439.9,905.4h-57.4c-65.1,0-107.6-9.6-130.1-29.3c-22.3-19.5-33.6-58.5-33.6-115.8v-82.3c0-37.3-6.5-63.3-19.4-77.2
	c-12.8-13.8-36.8-20.8-71.2-20.8h-23.7c-2.1,0-3.8-1.7-3.8-3.8v-73.1c0-2.1,1.7-3.8,3.8-3.8h23.7c34.4,0,58.4-6.9,71.2-20.6
	c12.9-13.7,19.4-39.8,19.4-77.3v-82.3c0-57.3,11.3-96.2,33.6-115.6c22.5-19.6,65-29.1,130.1-29.1h57.4c2.1,0,3.8,1.7,3.8,3.8v72.7
	c0,2.1-1.7,3.8-3.8,3.8h-47.1c-25.2,0-42.1,4.9-50.2,14.7c-8.3,10-12.5,32.9-12.5,68.3v80c0,42.7-6.6,73.3-19.5,91
	c-10.5,14.4-27.8,24.7-51.3,30.8c23.6,6.4,40.8,17.1,51.4,31.7c12.9,17.9,19.5,48.6,19.5,91v78.4c0,35.6,4.2,58.8,12.5,68.9
	c8.1,9.9,25,14.9,50.2,14.9h47.1c2.1,0,3.8,1.7,3.8,3.8v73.1C443.8,903.7,442,905.4,439.9,905.4z'/>
    <path fill='#ffffff' d='M726.3,777c-24.1,0-48.6-2.7-72.8-8.1c-24.2-5.3-47.9-13.4-70.5-23.9c-1.4-0.6-2.2-2-2.2-3.5v-91.8c0-1.4,0.8-2.7,2-3.4
	c1.2-0.7,2.8-0.6,3.9,0.2c25,16.6,49.6,29.1,73,37.2c23.3,8,46.6,12.1,69.4,12.1c22.5,0,40.1-4.7,52.3-14.1
	c12-9.2,17.9-21.9,17.9-38.6c0-12.4-4-23.5-12-32.9c-8.1-9.6-20.2-17.2-35.9-22.8L704.7,571c0,0,0,0,0,0
	c-48.5-17.2-81.8-35.6-99.2-54.7c-17.6-19.4-26.5-45.7-26.5-78.2c0-41.7,14.6-75,43.4-98.9c28.6-23.7,68.3-35.8,117.9-35.8
	c22.3,0,45,2.4,67.4,7.2c22.3,4.7,44.8,11.9,66.7,21.1c1.4,0.6,2.3,2,2.3,3.5v86.6c0,1.4-0.8,2.7-2,3.4c-1.2,0.7-2.7,0.6-3.9-0.2
	c-20.1-13.2-40.8-23.4-61.5-30.3c-20.6-6.9-41.3-10.3-61.5-10.3c-22.1,0-39.3,4.1-50.8,12.2c-11.3,7.9-16.8,19-16.8,33.9
	c0,11.4,4,20.6,12.2,28c8.5,7.7,26.9,16.5,54.8,26.1l41.1,14.4c39.2,13.4,68.6,31.5,87.3,53.8c18.8,22.5,28.4,51.1,28.4,85.2
	c0,46.4-15,81.5-44.6,104.6C829.8,765.4,785.1,777,726.3,777z'/>
    <path fill='#ffffff' d='M1344.1,768.6h-85.6c-2.1,0-3.8-1.7-3.8-3.8V431.6l-51.9,177.1c-0.5,1.6-2,2.8-3.7,2.8h-66.9c-1.7,0-3.2-1.1-3.7-2.7
	L1076,431.3v333.4c0,2.1-1.7,3.8-3.8,3.8h-85.6c-2.1,0-3.8-1.7-3.8-3.8V315.6c0-2.1,1.7-3.8,3.8-3.8h118.6c1.7,0,3.2,1.1,3.7,2.7
	l56.3,185l56-185c0.5-1.6,2-2.7,3.7-2.7h119.3c2.1,0,3.8,1.7,3.8,3.8v449.2C1348,766.8,1346.3,768.6,1344.1,768.6z'/>
    <path fill='#1a7fe5' d='M1537.5,905.4h-57.4c-2.1,0-3.8-1.7-3.8-3.8v-73.1c0-2.1,1.7-3.8,3.8-3.8h46.3c25.2,0,42.3-5.1,50.6-15.1
	c8.5-10.2,12.9-33.3,12.9-68.7v-78.4c0-43.1,6.4-72.8,19.5-91c10.5-14.6,27.6-25.2,51-31.7c-23.6-6.1-40.8-16.4-51.2-30.8
	c-13-17.9-19.3-47.7-19.3-91v-80c0-34.9-4.3-57.7-12.7-68c-8.3-10-25.3-15.1-50.8-15.1h-46.3c-2.1,0-3.8-1.7-3.8-3.8v-72.7
	c0-2.1,1.7-3.8,3.8-3.8h57.4c64.8,0,107.1,9.5,129.4,29.1c22.1,19.4,33.2,58.3,33.2,115.6v82.3c0,37.3,6.7,63.3,19.8,77.1
	c13.1,13.8,37.3,20.8,72,20.8h23.7c2.1,0,3.8,1.7,3.8,3.8v73.1c0,2.1-1.7,3.8-3.8,3.8h-23.7c-34.7,0-58.9,7.1-72,21
	c-13.1,14-19.8,39.9-19.8,77v82.3c0,57.2-11.2,96.2-33.2,115.7C1644.6,895.8,1602.3,905.4,1537.5,905.4z'/>
  </svg>
);
